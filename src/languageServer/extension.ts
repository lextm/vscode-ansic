...
export function activate(context: vscode.ExtensionContext, logger: Logger, disabled: boolean) {
    ...
    const languageServerPaths = [
        ".lsp/Server.exe",
        ".lsp/Server",
        "../ansi-c-antlr/Server/bin/Debug/netcoreapp3.1/Server.dll"
    ]
    ...
    if (serverModule != null) {
        ...
        let clientOptions: LanguageClientOptions = {
            ...
            documentSelector: [
                { language: 'ansic', scheme: 'file' },
                { language: 'ansic', scheme: 'untitled' }
            ],
            synchronize: {
                // Synchronize the setting section 'lspSample' to the server
                configurationSection: 'ansic',
        ...
        let disposable = new LanguageClient('ansic', 'ANSI C Language Server', serverOptions, clientOptions).start();
